...
class WaterRateScraper:
    ...
    def safe_pause(self, seconds):
        end_time = time.time() + seconds
        while time.time() < end_time:
            try:
                time.sleep(0.5)
            except KeyboardInterrupt:
                self.logger.warning("Pause interrupted, continuing...")
                break
    ...
    def run_all_scrapers(self):
        ...
        for batch_name, scraper_list in batches.items():
            self.logger.info(f"Starting {batch_name}...")

            for scraper in scraper_list:
                try:
                    scraper_name = getattr(scraper, "__name__", "lambda_scraper")
                    self.logger.info(f"Running {scraper_name}...")
                    scraper()
                except Exception as e:
                    self.logger.exception(f"Unhandled exception in {scraper_name}: {e}")

            self.logger.info(f"Finished {batch_name}.")
            self.safe_pause(3) # 3 second pause between batches
